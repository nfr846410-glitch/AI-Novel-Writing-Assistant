...
重大更新：写法开始以统一“写法合同”驱动正文、检测和修正，自动导演按卷继续时也会严格按目标卷范围续跑；Windows 桌面版也切到了 `0.2.1` 正式发布。

- 写法资产开始统一成更稳定的“写法合同”。正文生成、检测和修正文稿会围绕同一套叙事、角色、语言、节奏和反 AI 约束协同执行，绑定写法后更容易把风格真正落到正文里。
- 自动导演按卷继续执行更可靠了。选择继续某一卷时，系统会按目标卷的真实章节范围接着跑；如果前一卷还有未完成章节，会直接提示先补前序卷，避免悄悄退回默认前 10 章或继续到错误范围。
- 角色准备在模型输出不稳定时更容易补齐可用阵容。系统会优先把核心角色成员和关系分步补全，减少角色阶段因为一次结果不完整就把整条开书流程卡住的情况。
- 后台任务恢复和状态同步更稳了。数据库写入拥堵时会优先等待并重试，小说页和任务中心在继续、取消或重试自动导演后，也会更快同步到当前真实状态。
- Windows 桌面版正式进入 `0.2.1`。现在可以直接从 Releases 下载正式安装包，启动后继续使用模型配置引导、旧 `dev.db` 导入和安装版自动更新能力，不再需要按 Beta 口径理解发布链。
